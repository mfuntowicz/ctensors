...
int32_t ctensors_header_parse(ctensors_table_t* table, const char* header, const size_t hsize, const ctensors_flags_t flags)
{
    int32_t num_tensors = 0;
    int32_t num_allocated_tensors = 2048;
    table->tensors = malloc(num_allocated_tensors * sizeof(ctensors_tensor_t));

    if (!table->tensors) {
        return ctensors_headers_alloc_failed;
    }
    ...
    if (!document) {
        free(table->tensors);
        return ctensors_header_invalid_starting_char;
    }
    ...
    yyjson_obj_foreach(root, idx, max, key, val) {
        ...
        if (strcmp(tensor_name, "__metadata__") == 0) {
            if (flags & ctensors_skip_metadata) {
                continue;
            }
            // TODO: not impl yet ;
        }
        ...
        if (num_tensors >= num_allocated_tensors) {
            ...
            ctensors_tensor_t* new_tensors = realloc(table->tensors, num_allocated_tensors * sizeof(ctensors_tensor_t));
            if (!new_tensors) {
                // Cleanup on failure
        ...
        ctensors_tensor_t* tensor = &table->tensors[num_tensors];
        ...
        tensor->name = calloc(name_len + 1, sizeof(char));
        if (!tensor->name) {
            for (int32_t i = 0; i < num_tensors; i++) {
                free(table->tensors[i].shape);
            }
        ...
        strncpy(tensor->name, tensor_name, name_len);
        ...
        if (dtype_val && yyjson_is_str(dtype_val)) {
            ...
            tensor->dtype = ctensors_parse_dtype_from_str(dtype_str, dtype_len);
        } else {
            tensor->dtype = ctensors_unknown;
        }
    ...
    // Resize to th exact size if we over-allocated
    if (num_tensors < num_allocated_tensors && num_tensors > 0) {
        ctensors_tensor_t* final_tensors = realloc(table->tensors, num_tensors * sizeof(ctensors_tensor_t));
        if (final_tensors) {
            table->tensors = final_tensors;
        }
        // If realloc fails, we still have the original allocation which is fine
    }
